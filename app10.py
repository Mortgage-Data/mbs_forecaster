...
with st.expander("📊 Model Configuration", expanded=True):
    ...
    with row1_col2:
        # Loan Purpose Dropdown (no changes)
        loan_purpose_query = """
            SELECT DISTINCT loan_purpose FROM main.gse_sf_mbs
            WHERE is_in_bcpr3 AND prefix = 'CL'
            AND loan_purpose IS NOT NULL ORDER BY loan_purpose;
        """
        try:
            ...
            st.error(f"Could not load loan purpose list: {e}")
            st.stop()

    with row1_col3:
        # ADDED: Agency Dropdown
        agency_query = """
            SELECT DISTINCT agency FROM main.gse_sf_mbs
            WHERE agency IS NOT NULL ORDER BY agency;
        """
        try:
            agencies = [row[0] for row in con.execute(agency_query).fetchall()]
            agencies.insert(0, "All")
            selected_agency = st.selectbox("Select Agency", agencies, index=0)
        except Exception as e:
            st.error(f"Could not load agency list: {e}")
            st.stop()

    with row1_col4:
        # ADDED: Channel Dropdown
        channel_query = """
            SELECT DISTINCT channel FROM main.gse_sf_mbs
            WHERE channel IS NOT NULL ORDER BY channel;
        """
        try:
            channels = [row[0] for row in con.execute(channel_query).fetchall()]
            channels.insert(0, "All")
            selected_channel = st.selectbox("Select Channel", channels, index=0)
        except Exception as e:
            st.error(f"Could not load channel list: {e}")
            st.stop()
...
if selected_agency != "All":
    ...
    where_conditions.append(f"agency = '{safe_agency}'")
if selected_channel != "All":
    safe_channel = selected_channel.replace("'", "''")
    where_conditions.append(f"channel = '{safe_channel}'")
...
# Construct a more descriptive filter string
filter_parts = [selected_loan_purpose, selected_agency, selected_channel]
filter_desc = ", ".join(p for p in filter_parts if p != "All")
if not filter_desc:
    filter_desc = "All"

st.info(f"📊 **{selected_seller} ({filter_desc})**: {seller_info[0]:,} total loan observations across {seller_info[1]} months "
...
with st.spinner("Loading and preparing data..."):
    ...
    seller_level_query = f"""
        ...
        prepayment_flows AS (
            ...
            WHERE is_in_bcpr3 AND prefix = 'CL' AND {where_clause}
            AND as_of_month >= '2021-10-01'
            -- No exclusion of loan_correction_indicator='pri' here, so we capture the payoff events
            GROUP BY DATE_TRUNC('month', as_of_month)
        ),
        ...
        cohort_characteristics AS (
            SELECT
                ...
                SUM(1) as cohort_loan_count,
                SUM(current_investor_loan_upb) as cohort_total_upb
            FROM main.gse_sf_mbs a
            WHERE is_in_bcpr3 AND prefix = 'CL'
        ...
        seller_monthly AS (
            SELECT
                ...
                cd.cohort_loan_count,
                cd.cohort_total_upb,
                CASE WHEN cd.cohort_smm > 0 THEN 1 - POWER(1 - cd.cohort_smm, 12) ELSE 0 END as cohort_cpr,
                cc.cohort_cpr3
            ...
            LEFT JOIN cohort_data cd ON ms.month = cd.month
            LEFT JOIN cohort_cpr3 cc ON ms.month = cc.month
        )
        SELECT
            sm.*,
            CASE WHEN sm.cohort_cpr3 > 0 THEN (sm.cpr3 / sm.cohort_cpr3 * 100) ELSE 0 END as bcpr3, -- Use sm.cohort_cpr3
            ...
            LAG(CASE WHEN sm.cohort_cpr3 > 0 THEN (sm.cpr3 / sm.cohort_cpr3 * 100) ELSE 0 END, 1) OVER (ORDER BY sm.month) as bcpr3_1m_lag,
            LAG(CASE WHEN sm.cohort_cpr3 > 0 THEN (sm.cpr3 / sm.cohort_cpr3 * 100) ELSE 0 END, 3) OVER (ORDER BY sm.month) as bcpr3_3m_lag,
            AVG(CASE WHEN sm.cohort_cpr3 > 0 THEN (sm.cpr3 / sm.cohort_cpr3 * 100) ELSE 0 END) OVER (ORDER BY sm.month ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING) as bcpr3_6m_avg,
    ...
    if model_type == "LightGBM" or model_type == "Auto-Select":
        status_container.info("📊 Loading loan-level data for diagnostics...")
        loan_data = con.execute(loan_level_query).df()
        if len(loan_data) > 0:
            status_container.success(f"✅ Loaded {len(loan_data):,} loan-month observations for diagnostics.")
        else:
            st.warning("⚠️ No loan-level data found for this period.")

    status_container.info("📊 Loading seller-level aggregate data for modeling...")
    try:
    ...
    if len(seller_data) > 0:
        ...
        if final_rows > 0:
            status_container.success(f"✅ Prepared {final_rows} months of clean seller-level data for modeling.")
            if initial_rows > final_rows:
                status_container.info(f"ℹ️ Dropped {initial_rows - final_rows} row(s) due to missing data.")
        else:
...
with st.spinner("Training models and generating forecasts..."):
    ...
    if LIGHTGBM_AVAILABLE and suggested_model == "gradient_boosting":
        ...
        status_container.success(f"✅ Auto-ARIMA model trained: {model.order}")
...
# --- Prep KPI data ---
latest_row = seller_data.iloc[-1]
next_month_forecast = forecast_df['forecast'].iloc[0]

# Bug Fix: Conditionally scale the current value for display
current_raw_value = latest_row[target_col]
current_display_value = current_raw_value if target_variable == 'BCPR3' else current_raw_value * 100

# Calculate delta using the correctly scaled current value
delta = next_month_forecast - current_display_value
...
# --- Layout: Build KPIs row-by-row for alignment ---
# Headers
col1, col2 = st.columns(2)
with col1:
    st.markdown("##### 🏢 Seller Performance")
with col2:
    st.markdown("##### 벤 Peer Cohort Benchmark")

# Row 1: Active Model & Predicted CPR
col1, col2 = st.columns(2)
with col1:
    st.metric(label="Active Model", value=primary_model)
    st.metric(
        label=f"Predicted {target_variable} (Next Month)",
...
with col1:
    ...
    st.metric(label=f"Current {target_variable}", value=f"{current_display_value:.2f}%")
with col2:
    if target_variable in ['CPR', 'CPR3']:
        target_cohort_col = 'cohort_cpr' if target_variable == 'CPR' else 'cohort_cpr3'
        if target_cohort_col in latest_row and not pd.isna(latest_row[target_cohort_col]):
             st.metric(label=f"Current Cohort {target_variable}", value=f"{latest_row[target_cohort_col]*100:.2f}%")
        else:
             st.info(f"Cohort {target_variable} data not available for this month.")
    elif target_variable == 'BCPR3':
        st.metric(label="Current Cohort BCPR", value="100.00", help="The cohort average is 100 by definition.")

# Row 3: Loan Count
col1, col2 = st.columns(2)
with col1:
    st.metric(label="Loan Count (Latest Month)", value=format_number(latest_row['loan_count']))
with col2:
    st.metric(label="Cohort Loan Count (Latest Month)", value=format_number(latest_row['cohort_loan_count']))

# Row 4: Total UPB
col1, col2 = st.columns(2)
with col1:
    st.metric(label="Total UPB (Latest Month)", value=format_currency(latest_row['total_upb']))
with col2:
    st.metric(label="Cohort Total UPB (Latest Month)", value=format_currency(latest_row['cohort_total_upb']))
