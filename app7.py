    ...
    seller_level_query = f"""
        ...
        seller_cpr3 AS (
            SELECT 
                month,
                CASE WHEN smm3 > 0 
                    THEN 1 - POWER(1 - smm3, 12) 
                    ELSE 0 END as seller_cpr3
            FROM smm_averages
        ),
        cohort_data AS (
            SELECT 
        ...
        cohort_smm_averages AS (
            SELECT 
                month,
                cohort_smm,
                (cohort_smm + LAG(cohort_smm, 1) OVER (ORDER BY month) + LAG(cohort_smm, 2) OVER (ORDER BY month)) / 3 as cohort_smm3
            FROM cohort_data
        ),
        cohort_cpr3 AS (
            SELECT 
                month,
                CASE WHEN cohort_smm3 > 0 
                    THEN 1 - POWER(1 - cohort_smm3, 12) 
                    ELSE 0 END as cohort_cpr3
