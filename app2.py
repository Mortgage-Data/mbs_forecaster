...
def prepare_lightgbm_features(loan_data, seller_data, target_col='cpr'):
    ...
    This leverages individual loan characteristics while predicting CPR.
    """
    ...
    df['jumbo_loan'] = (df['loan_balance'] > 647200).astype(int)
    
    # Temporal features
    df['month'] = df.index.month
    df['year'] = df.index.year
    df['time_index'] = df.groupby('loan_id').cumcount()
...
with st.spinner("Loading data for selected modeling approach..."):
    ...
    loan_level_query = """
        ...
        SELECT *,
            ...
            loan_rate - pmms30 as refi_incentive,
            ABS(loan_rate - pmms30_1m_lag) as rate_volatility,
...
with st.expander("📊 Data Summary", expanded=False):
    if loan_data is not None and len(loan_data) > 0:
        ...
        if 'current_delinquency_status' in loan_data.columns:
            dist_col4.metric("Delinquent Loans", f"{(loan_data['current_delinquency_status'] != '0').mean():.1%}")
    else:
        col1, col2, col3, col4, col5, col6 = st.columns(6)
