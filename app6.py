...
def prepare_seller_level_features(seller_data, target_col='cpr'):
    """
    Feature engineering for seller-level aggregated data.
    """
    df = seller_data.copy()
...
# Escape seller name and build WHERE clause
    ...
    seller_level_query = f"""
        ...
        smm_averages AS (
            SELECT 
                month,
                smm,
                (smm + LAG(smm, 1) OVER (ORDER BY month) + LAG(smm, 2) OVER (ORDER BY month)) / 3 as smm3
            FROM base_data
        ),
        bcpr_data AS (
            SELECT 
        ...
        SELECT 
            ...
            LAG(bcpr.bcpr3, 1) OVER (ORDER BY sm.month) as bcpr3_1m_lag,
            LAG(bcpr.bcpr3, 3) OVER (ORDER BY sm.month) as bcpr3_3m_lag,
            AVG(bcpr.bcpr3) OVER (ORDER BY sm.month ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING) as bcpr3_6m_avg,
        ...
        LEFT JOIN bcpr_data bcpr ON sm.month = bcpr.month
        ORDER BY sm.month
    """
    ...
    if len(seller_data) > 0:
        ...
        if target_col not in seller_data.columns:
            st.error(f"❌ Target column '{target_col}' not found in data for {selected_seller} ({selected_loan_purpose}). "
                     f"This may occur if no BCPR3-eligible loans exist for the selected filters. "
                     f"Try selecting a different loan purpose or seller.")
            st.stop()
        
        # Fill null BCPR3 values with 0 to avoid dropping rows
        if target_col == 'bcpr3':
